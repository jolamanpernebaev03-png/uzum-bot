    ...
    if _cache_fresh(context):
        ...
        try:
            ...
            if "UZUM_TOKEN_EXPIRED" in str(e):
                await update.message.reply_text("⚠️ Token expired. Auto-refreshing... please wait 30 seconds.")
                try:
                    from refresh_and_push import refresh_and_push
                    success = await asyncio.get_event_loop().run_in_executor(None, refresh_and_push)
                    if success:
                        await update.message.reply_text("✅ Token refreshed! Retrying report...")
                        await _run_report(update, context, budget)
                    else:
                        await update.message.reply_text("❌ Token refresh failed. Please update UZUM_BEARER_TOKEN manually in Railway Variables.")
                except Exception as re:
                    await update.message.reply_text(f"❌ Token refresh error: {re}")
                return
            raise
        if not products:
